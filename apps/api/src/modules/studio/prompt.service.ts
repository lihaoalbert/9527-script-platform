...
【宪法校验（生成前必须逐条确认）】
1. 新登场的配角是否在角色清单中？如果不在，先在content中说明需要添加此角色
2. 本集出现的所有人名，与前几集是否一致？（例：ep1叫"同学小雅"，ep3就不能叫"病友小雅"）
...
1. 本集出现的所有配角名字，是否与前面已锁定集的同一人一致？（如：不能ep1叫"同学小雅"ep3叫"病友小雅"）
2. 角色的积分/信用分/等级等数值，是否与前集连贯？有没有跳变？
3. 世界硬规则（信用分阈值、平台惩罚机制等）是否被遵守？
4. 大纲约定的本集钩子和核心事件是否兑现？
5. 规划书中的角色是否需要补充？如有新角色未入清单，在suggestions中提醒用户
6. 大纲/世界观/人物是否存在遗漏或空白？如有，在suggestions中明确指出
